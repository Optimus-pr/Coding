...
vector<int> findTwoElement(vector<int>& arr) {
    ...
    for(int i=0;i<n;i++)
    {
        int ele=arr[i]%add;//this is to extract the original element present in that place 
        arr[ele]+=add;//this is to keep track of cnt when we do /by add we will get the cnt
    }
    for(int i=0;i<n;i++)
    {
        if(arr[i]/add==2)
